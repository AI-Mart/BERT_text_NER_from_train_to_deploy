...
import argparse
import os
import sys#change by mart 11-3

__all__ = ['get_args_parser']

def get_args_parser():
    from .bert_lstm_ner import __version__#同级文件夹的文件调用，直接用.代替当前目录
    parser = argparse.ArgumentParser()
    if os.name == 'nt':
        # bert_path = 'F:\chinese_L-12_H-768_A-12'
        # root_path = r'C:\workspace\python\BERT-BiLSTM-CRF-NER'
        # root_path = r"E:\AI_train\Z_NER\ws_govland_11_20_BERT-BiLSTM-CRF-NER"  # change by mart 11-3  一定要记住sys函数在不同的python文件里面运行得到的结果是不一样的 这个sys[]慎重用 容易出错不用
        root_path = os.path.abspath(os.path.dirname(__file__)).split("bert_base")[0]  # chnage by mart2020-11-20防止移动文件由于路径问题出错
        bert_path = os.path.join(root_path,'roeberta_zh_L-12_H-768_A-12')#change by mart 11-3 一定要记住sys函数在不同的python文件里面运行得到的结果是不一样的
    else:
    ...
    group2.add_argument('-max_seq_length', type=int, default=420,#change by mart 20-11-18
    ...
    group2.add_argument('-save_summary_steps', type=int, default=1,#change by mart 20-11-28
    ...
    group2.add_argument('-clean', type=bool, default=False)#change by mart 20-11-28
